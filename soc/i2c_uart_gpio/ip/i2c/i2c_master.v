...
`endif
  ...
  begin
    ...
    begin
      ...
      case(state)
        ...
        START:
        begin
          if(scl_prev & scl_curr & clk_i2c_cntr == START_IND_SETUP)
          begin   //check that scl is high, and that a necessary wait time is held
        ...
        begin
          ...
          begin
            ...
            begin   //Must wait minimum setup time
              scl_is_high <= 1'b0;
              reg_sda_o <= 1'b0;
              state <= SLAVE_ADDR;
              byte_sr <= addr;
        ...
        begin
          ...
          begin
            ...
            begin
              if(!sda_prev)
              begin      //checking for the ack condition (its low)
                state <= next_state;
        ...
        STOP:
        begin
          if(!scl_curr & scl_prev & !rw)
          begin //negedge only if we are writing
          ...
          if(scl_curr & scl_prev & en_end_indicator)
          begin
            scl_is_high <= 1'b1;
          ...
          if(scl_is_high)
          begin
            if(clk_i2c_cntr == STOP_IND_SETUP)
            begin
              reg_sda_o <= 1'b1;
              state <= RELEASE_BUS;
              scl_is_high <= 1'b0;
   ...
   */
  always@(negedge i_clk or negedge reset_n)
  begin
    if(!reset_n)
    begin
      {sda_curr, sda_prev} <= 0;
